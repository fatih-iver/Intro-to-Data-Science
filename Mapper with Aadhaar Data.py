...
def mapper():
    ...
    indexOfDistrict = header.index('District')
    indexOfAadhaarGenerated = header.index('Aadhaar generated')

    for line in sys.stdin:
        row = line.strip().split(',')
        if len(row) == len(header):
            print "{0}/t{1}".format(row[indexOfDistrict], row[indexOfAadhaarGenerated])
